import 'dart:convert';

import 'package:flutter/material.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:sochat_client/modules/keys/key.dart';
import 'package:sochat_client/modules/network/message_packet.dart';
import 'package:sochat_client/modules/network/tcp_socket_service.dart';
import 'package:sochat_client/modules/users/user.dart';

final authServiceProvider = NotifierProvider<AuthService, AuthState>(
  AuthService.new,
);

const _unset = Object();

class AuthState {
  final User? currentUser;

  const AuthState(this.currentUser);

  AuthState copyWith({Object? currentUser = _unset}) {
    return AuthState(
      currentUser == _unset ? this.currentUser : currentUser as User?,
    );
  }
}

class AuthService extends Notifier<AuthState> {
  String? token;

  User? get currentUser => state.currentUser;
  late final TcpSocketService _tcpSocketService;

  @override
  AuthState build() {
    ref.watch(tcpSocketProvider.future).then((tcp) {
      _tcpSocketService = tcp;
    });
    return AuthState(null);
  }

  void setCurrentUser(User? user) {
    state = state.copyWith(currentUser: user);
  }

  Future<MessagePacket> login(
    BuildContext context,
    String username,
    KeyP keyPair,
    String ip,
    WidgetRef ref,
  ) async {
    final response = await _tcpSocketService.sendRequest(MessagePacket(type: "auth_login", payload: {"username": username}));

    return response;
  }

  Future<MessagePacket> verify(
    BuildContext context,
    String username,
    KeyP keyPair,
    String challenge,
    ip,
    WidgetRef ref,
  ) async {
    var signature = await keyPair.sign(utf8.encode(challenge));
    var signatureBase64 = base64Encode(signature);

    MessagePacket packet = MessagePacketBuilder()
        .type("auth_login_verify")
        .put("username", username)
        .put("signature", signatureBase64)
        .put("challenge", challenge)
        .build();

    final response = await _tcpSocketService.sendRequest(packet);

    return response;
  }

  Future<void> register(String username, KeyP keyPair, String ip) async {
    await _tcpSocketService.connect();

    MessagePacket packet = MessagePacketBuilder()
        .type('auth_register')
        .put('username', username)
        .put('ed25519PublicKey', base64Encode(keyPair.x509ed25519PublicKey()))
        .put('x25519PublicKey', base64Encode(keyPair.x509x25519PublicKey()))
        .build();

    await _tcpSocketService.sendRequest(packet);
  }
}
